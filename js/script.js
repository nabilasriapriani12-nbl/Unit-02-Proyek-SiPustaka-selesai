...
if (elemenFormPinjam !== null) {
    ...
    function tampilkanDaftarPeminjaman() {
        ...
        html += '<thead class="table-light"><tr><th>Buku</th><th>Peminjam</th><th>Kembali</th><th>Aksi</th></tr></thead><tbody>';
        ...
        for (var j = 0; j < daftarPeminjaman.length; j++) {
            ...
            var tglKembaliObj = new Date(p.tanggalKembaliRaw);
            tglKembaliObj.setHours(0,0,0,0);
            
            var selisihMilidetik = tglKembaliObj.getTime() - tglSekarang.getTime();
            ...
            if (selisihHari <= 3) {
                kelasBaris = 'table-danger';
                badgeJatuhTempo = ' <span class="badge bg-danger">⚠️ Segera Kembalikan</span>';
            }
            
            html += '<tr class="' + kelasBaris + '">';
            html += '<td><small class="fw-semibold">' + p.judulBuku + '</small></td>';
            html += '<td><small>' + p.namaPeminjam + '</small></td>';
            html += '<td><small>' + p.tanggalKembali + badgeJatuhTempo + '</small></td>';
            html += '<td><button class="btn btn-outline-danger btn-sm style="padding: .2rem .4rem; font-size: .75rem;" onclick="hapusPeminjaman(' + j + ')">Kembalikan</button></td>';
            html += '</tr>';
        }
    ...
    elemenFormPinjam.addEventListener('submit', function(event) {
        ...
        // Simpan format string YYYY-MM-DD asli untuk kalkulasi Date di kemudian hari
        var formatRawKembali = tglKembali.toISOString().split('T')[0];
        ...
        var peminjamanBaru = {
            ...
            tanggalKembaliRaw: formatRawKembali, // Properti mentah untuk Tantangan 3
            durasi: durasi
        };
...
if (elemenAccordion !== null) {
    ...
    function tampilkanFaq() {
        var kataCari = document.getElementById('inputCariFaq').value.toLowerCase();
        var htmlAccordion = '';
        var jumlahCocok = 0;

        for (var i = 0; i < dataFaq.length; i++) {
            var faq = dataFaq[i];
            var pertanyaamLower = faq.pertanyaan.toLowerCase();
            var jawabanLower = faq.jawaban.toLowerCase();

            if (pertanyaamLower.indexOf(kataCari) !== -1 || jawabanLower.indexOf(kataCari) !== -1) {
                jumlahCocok++;
        ...
        var pesanKosongFaq = document.getElementById('pesanKosongFaq');
        if (pesanKosongFaq) {
            if (jumlahCocok === 0) {
                pesanKosongFaq.classList.remove('d-none');
            } else {
                pesanKosongFaq.classList.add('d-none');
            }
        }
    }

    var inputFaq = document.getElementById('inputCariFaq');
    if (inputFaq) {
        inputFaq.addEventListener('input', tampilkanFaq);
    }
    tampilkanFaq();
}
